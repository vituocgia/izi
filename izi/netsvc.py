...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('izi.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
            cr.autocommit(True)
...
DEFAULT_LOG_CONFIGURATION = [
    'izi.http.rpc.request:INFO',
    'izi.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['izi:DEBUG', 'izi.sql_db:INFO', 'izi.http.rpc:DEBUG'],
    'debug_rpc': ['izi:DEBUG', 'izi.sql_db:INFO', 'izi.http.rpc.request:DEBUG'],
    'debug': ['izi:DEBUG', 'izi.sql_db:INFO'],
    'debug_sql': ['izi.sql_db:DEBUG'],
    'info': [],
    'warn': ['izi:WARNING', 'werkzeug:WARNING'],
    'error': ['izi:ERROR', 'werkzeug:ERROR'],
    'critical': ['izi:CRITICAL', 'werkzeug:CRITICAL'],
}
