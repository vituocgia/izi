...
class _LogTracer(object):
    ...
    def tracer(self, frame, event, arg):
        ...
        if isinstance(in_self, (izi.sql_db.Cursor, izi.sql_db.TestCursor, izi.sql_db.LazyCursor)):
            return self.tracer
...
def profile(method=None, whitelist=None, blacklist=(None,), files=None,
    ...
    """
        ...
        all methods for all izi models by applying the optional filters.
        ...
        :param list blacklist: list model names to remove from the log
                        (Default: remove non izi model from the log: [None])
        ...
        .. code-block:: python
          ...
          class SaleOrder(models.Model):
            ...
            ...
            @api.multi
            @profile()                  # log all methods for all izi models
            def unlink(self):
            ...
            @profile(whitelist=['sale.order', 'ir.model.data'])
            def action_quotation_send(self):
            ...
            @profile(files=['/home/openerp/izi/izi/addons/sale/models/sale.py'])
            def write(self):
            ...
    ...
    def _iziProfile(method, *args, **kwargs):
    ...
    if not method:
        return lambda method: decorator(_iziProfile, method)

    wrapper = decorator(_iziProfile, method)
    return wrapper
