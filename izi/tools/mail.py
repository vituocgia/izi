...
import izi
from izi.loglevels import ustr
from izi.tools import pycompat, misc
...
def email_send(email_from, email_to, subject, body, email_cc=None, email_bcc=None, reply_to=False,
    ...
    if not cr:
        ...
        if db_name:
            local_cr = cr = izi.registry(db_name).cursor()
        else:
            raise Exception("No database cursor found, please pass one explicitly")

    # Send Email
    try:
        mail_server_pool = izi.registry(cr.dbname)['ir.mail_server']
        res = False
