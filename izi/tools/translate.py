...
class GettextAlias(object):
    ...
    def _get_db(self):
        ...
        if db_name:
            return izi.sql_db.db_connect(db_name)
    ...
    def _get_lang(self, frame):
        ...
        if not lang:
            ...
            if not lang:
                ...
                if cr and uid:
                    env = izi.api.Environment(cr, uid, {})
        ...
        try:
            ...
            if lang:
                ...
                if cr:
                    ...
                    env = izi.api.Environment(cr, izi.SUPERUSER_ID, {})
...
def trans_generate(lang, modules, cr):
    ...
    path_list = [(path, True) for path in izi.modules.module.ad_paths]
        ...
        for root, dummy, files in walksymlinks(path):
            ...
            if fnmatch.fnmatch(root, '*/static/src/xml*'):
                for fname in fnmatch.filter(files, '*.xml'):
                    babel_extract_terms(fname, path, root, 'izi.tools.translate:babel_extract_qweb',
                                        extra_comments=[WEB_TRANSLATION_COMMENT])
            if not recursive:
...
def trans_load_data(cr, fileobj, fileformat, lang, lang_name=None, verbose=True, module_name=None, context=None):
    ...
    env = izi.api.Environment(cr, izi.SUPERUSER_ID, context or {})
    Lang = env['res.lang']
    Translation = env['ir.translation']
