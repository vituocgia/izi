...
def _get_idref(self, env, model_str, idref):
    idref2 = dict(idref,
                  ...
                  version=izi.release.major_version,
                  ref=self.id_get,
                  pytz=pytz)
    if model_str:
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval:
            ...
            try:
                ...
                logging.getLogger('izi.tools.convert.init').error(
        ...
        return izi.api.call_kw(model, method, args, {})
    elif node.tag == "test":
        return node.text
...
form: module.record_id""" % (xml_id,)
    ...
    def parse(self, de, mode=None):
        roots = ['openerp','data','izi']
        if de.tag not in roots:
            raise Exception("Root xml tag must be <openerp>, <izi> or <data>.")
        for rec in de:
            if rec.tag in roots:
                self.parse(rec, mode)
    ...
    def __init__(self, cr, module, idref, mode, report=None, noupdate=False, xml_filename=None):
        self.mode = mode
        self.module = module
        self.env = izi.api.Environment(cr, SUPERUSER_ID, {})
        self.cr = cr
