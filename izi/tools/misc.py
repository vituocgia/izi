...
from izi.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if izi.tools.config['db_host']:
        env['PGHOST'] = izi.tools.config['db_host']
    if izi.tools.config['db_port']:
        env['PGPORT'] = str(izi.tools.config['db_port'])
    if izi.tools.config['db_user']:
        env['PGUSER'] = izi.tools.config['db_user']
    if izi.tools.config['db_password']:
        env['PGPASSWORD'] = izi.tools.config['db_password']
    return env
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = izi.modules.module.get_resource_path('base', 'res', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('izi.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('izi.foo.bar'):
            do_suff()
