...
import izi.sql_db
import izi.tools
from izi.sql_db import db_connect
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not izi.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and izi.tools.config.verify_admin_password(passwd):
        return True
    raise izi.exceptions.AccessDenied()

# This should be moved to izi.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None):
    try:
        db = izi.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            izi.modules.db.initialize(cr)
            izi.tools.config['load_language'] = lang
            cr.commit()

        registry = izi.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = izi.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = izi.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = izi.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = izi.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    izi.sql_db.close_db(db_original_name)
    db = izi.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = izi.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = izi.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = izi.tools.config.filestore(db_original_name)
    to_fs = izi.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    izi.modules.registry.Registry.delete(db_name)
    izi.sql_db.close_db(db_name)
    ...
    fs = izi.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'izi_dump': '1',
        'db_name': cr.dbname,
        'version': izi.release.version,
        'version_info': izi.release.version_info,
        'major_version': izi.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with izi.tools.osutil.tempdir() as dump_dir:
            filestore = izi.tools.config.filestore(db_name)
            ...
            izi.tools.exec_pg_command(*cmd)
            if stream:
                izi.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                izi.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = izi.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with izi.tools.osutil.tempdir() as dump_dir:
        ...
        if izi.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = izi.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = izi.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    izi.modules.registry.Registry.delete(old_name)
    izi.sql_db.close_db(old_name)
    ...
    old_fs = izi.tools.config.filestore(old_name)
    new_fs = izi.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    izi.tools.config.set_admin_password(new_password)
    izi.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        izi.tools.config['update']['base'] = True
        izi.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@izi.tools.mute_logger('izi.sql_db')
...
def list_dbs(force=False):
    if not izi.tools.config['list_db'] and not force:
        raise izi.exceptions.AccessDenied()

    if not izi.tools.config['dbfilter'] and izi.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, izi will not
        ...
        res = sorted(db.strip() for db in izi.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            db_user = izi.tools.config["db_user"]
            if not db_user and os.name == 'posix':
                import pwd
                db_user = pwd.getpwuid(os.getuid())[0]
            if not db_user:
                cr.execute("select usename from pg_user where usesysid=(select datdba from pg_database where datname=%s)", (izi.tools.config["db_name"],))
                res = cr.fetchone()
            ...
            res = [izi.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
            res = []
    res.sort()
    return res
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of izi
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if izi.tools.table_exists(cr, 'ir_module_module'):
                ...
                incompatible_databases.append(database_name)
        # release connection
        izi.sql_db.close_db(database_name)
...
def exp_list_lang():
    return izi.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(izi.tools.config['root_path'], 'addons/base/res/res_country_data.xml')).getroot()
