...
import izi
from izi.exceptions import UserError, ValidationError, QWebException
from izi.models import check_method_name
from izi.tools.translate import translate
...
def dispatch(method, params):
    ...
    registry = izi.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            try:
                cr = izi.sql_db.db_connect(dbname).cursor()
        ...
        while True:
            try:
                if izi.registry(dbname)._init and not izi.tools.config['test_enable']:
                    raise izi.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    recs = izi.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    return izi.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with izi.registry(db).cursor() as cr:
