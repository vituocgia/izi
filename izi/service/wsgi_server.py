...
# User code must use the exceptions defined in ``izi.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, izi.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, izi.tools.ustr(e.name))
    ...
    elif isinstance(e, izi.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = izi.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, izi.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(izi.tools.ustr(e), formatted_info)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(izi.tools.exception_to_unicode(e), formatted_info)
...
def _patch_xmlrpc_marshaller():
    ...
    # Historically, izi is sending binary as base64 string.
    ...
    def dump_bytes(marshaller, value, write):
        marshaller.dump_unicode(izi.tools.ustr(value), write)
...
def wsgi_xmlrpc(environ, start_response):
    ...
    if environ['REQUEST_METHOD'] == 'POST' and environ['PATH_INFO'].startswith('/xmlrpc/'):
        ...
        try:
            result = izi.http.dispatch_rpc(service, method, params)
...
def application_unproxied(environ, start_response):
    ...
    # izi.service.web_services.objects_proxy.dispatch().
    ...
    with izi.api.Environment.manage():
        ...
        for handler in [wsgi_xmlrpc, izi.http.root]:
