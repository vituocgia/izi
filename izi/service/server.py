...
import izi
from izi.modules.module import run_unit_tests, runs_post_install
from izi.modules.registry import Registry
from izi.release import nt_service_name
from izi.tools import config
from izi.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'izi.service.http.request.%s' % (me.ident,)
...
class FSWatcher(object):
    def __init__(self):
        ...
        for path in izi.modules.module.ad_paths:
            ...
            if not event.is_directory:
                ...
                if path.endswith('.py'):
                    try:
                        ...
                        if not getattr(izi, 'phoenix', False):
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from izi.addons.base.ir.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = izi.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(izi.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="izi.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="izi.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        _logger.debug('--')
        izi.modules.registry.Registry.delete_all()
        logging.shutdown()
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('izi: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(izi.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = izi.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('izi.netsvc.rpc.request')
...
def load_server_wide_modules():
    for m in izi.conf.server_wide_modules:
        try:
            izi.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if izi.tools.osutil.is_running_as_nt_service():
...
def load_test_file_yml(registry, test_file):
    with registry.cursor() as cr:
        izi.tools.convert_yaml_import(cr, 'base', open(test_file, 'rb'), 'test', {}, 'init')
    ...
    for mod_name, mod_mod in list(sys.modules.items()):
        if mod_mod:
            ...
            if test_path == mod_path:
                ...
                stream = izi.modules.module.TestStream()
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_file']:
                ...
                with izi.api.Environment.manage():
            ...
            if config['test_enable']:
                ...
                _logger.info("All post-tested in %.2fs, %s queries",
                             time.time() - t0, izi.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the izi http server and cron processor.
    """
    global server

    load_server_wide_modules()
    izi.service.wsgi_server._patch_xmlrpc_marshaller()

    if izi.evented:
        server = GeventServer(izi.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(izi.service.wsgi_server.application)
        ...
        server = ThreadedServer(izi.service.wsgi_server.application)
