...
__author__ = izi.release.author
__version__ = izi.release.version

# Also use the `izi` logger for the main script.
_logger = logging.getLogger('izi')
...
def report_configuration():
    ...
    """
    config = izi.tools.config
    _logger.info("izi version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', izi.modules.module.ad_paths)
...
def setup_pid_file():
    ...
    """
    config = izi.tools.config
    if not izi.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = izi.modules.registry.Registry.new(dbname)
        with izi.api.Environment.manage():
            with registry.cursor() as cr:
                izi.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with izi.api.Environment.manage():
        with registry.cursor() as cr:
            izi.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    izi.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                izi.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except izi.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        izi.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = izi.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the izi server (default command)"""
    def run(self, args):
        main(args)
