...
class Shell(Command):
    """Start izi in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        izi.cli.server.report_configuration()
        izi.service.server.start(preload=[], stop=True)
        ...
        with izi.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = izi.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = izi.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
