...
# *broken*
NO_POSTMORTEM = (izi.osv.orm.except_orm,
                 izi.exceptions.AccessError,
                 izi.exceptions.ValidationError,
                 izi.exceptions.MissingError,
                 izi.exceptions.AccessDenied,
                 izi.exceptions.Warning,
                 izi.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                izi.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = izi.service.common.dispatch
        elif service_name == 'db':
            dispatch = izi.service.db.dispatch
        elif service_name == 'object':
            dispatch = izi.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                izi.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                izi.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except izi.exceptions.DeferredException as e:
        _logger.exception(izi.tools.exception_to_unicode(e))
        izi.tools.debugger.post_mortem(izi.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(izi.tools.exception_to_unicode(e))
        izi.tools.debugger.post_mortem(izi.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all izi Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in izi.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~izi.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~izi.api.Environment` bound to current request. """
        if self._env is None:
            self._env = izi.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            izi.tools.debugger.post_mortem(
                izi.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        izi implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <izi.http.WebRequest.csrf_token`, the
          :data:`~izi.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          izi) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, (izi.exceptions.Warning, SessionExpiredException,
                                          izi.exceptions.except_orm, werkzeug.exceptions.NotFound)):
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "izi Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "izi Session Expired"
...
def serialize_exception(e):
    ...
    if isinstance(e, izi.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, izi.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, izi.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, izi.exceptions.AccessError):
        tmp["exception_type"] = "access_error"
    elif isinstance(e, izi.exceptions.MissingError):
        tmp["exception_type"] = "missing_error"
    elif isinstance(e, izi.exceptions.AccessDenied):
        tmp["exception_type"] = "access_denied"
    elif isinstance(e, izi.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, izi.exceptions.except_orm):
...
izi URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.izi.asia/documentation/11.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through izi via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["izi", "addons"]:
            module = ""
        else:
...
def routing_map(modules, nodb_only, converters=None):
    ...
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('izi.addons.') and c.__module__.split(".")[2] in modules
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or izi.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = izi.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        return routing_map([''] + izi.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        # The izi_PRELOAD_ADDONS environment variable is available for version 11.0 only.
        ...
        # activation in such case ` izi_PRELOAD_ADDONS=no ` should be used.
        # Note: izi versions > 11.0 does not preload addons anymore.
        preload_addons = os.environ.get('izi_PRELOAD_ADDONS', 'yes') == 'yes'

        # TODO should we move this to ir.http so that only configured modules are served ?
        statics = {}
        for addons_path in izi.modules.module.ad_paths:
            for module in sorted(os.listdir(str(addons_path))):
                if module not in addons_module:
                    ...
                    if manifest_path and os.path.isdir(path_static):
                        ...
                        m = None
                        if 'izi.addons' in sys.modules and preload_addons:
                            m = __import__('izi.addons.' + module)
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        izi.registry(db).check_signaling()
                        with izi.tools.mute_logger('izi.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = izi.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if izi.tools.config['dbfilter']:
        r = izi.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif izi.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, izi will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in izi.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = izi.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = izi.tools.ustr(filename)
