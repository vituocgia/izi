...
from izi.tools import (assertion_report, config, existing_tables,
                        lazy_classproperty, lazy_property, table_exists,
                        topological_sort, OrderedSet)
from izi.tools.lru import LRU
...
class Registry(Mapping):
    ...
    def __new__(cls, db_name):
        ...
        with cls._lock:
            try:
                ...
                # izi.service.wsgi_server.application
        ...
        with cls._lock:
            with izi.api.Environment.manage():
                ...
                try:
                    ...
                    try:
                        izi.modules.load_modules(registry._db, force_demo, status, update_module)
                    except Exception:
                        izi.modules.reset_modules_state(db_name)
                        raise
                except Exception:
    ...
    def init(self, db_name):
        ...
        self._db = izi.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            has_unaccent = izi.modules.db.has_unaccent(cr)
            if izi.tools.config['unaccent'] and not has_unaccent:
                _logger.warning("The option --unaccent was given but no unaccent() function was found in database.")
            self.has_unaccent = izi.tools.config['unaccent'] and has_unaccent
    ...
    def init_models(self, cr, model_names, context):
        ...
        env = izi.api.Environment(cr, SUPERUSER_ID, context)
