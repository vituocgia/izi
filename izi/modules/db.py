...
def is_initialized(cr):
    ...
    """
    return izi.tools.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    f = izi.modules.get_module_resource('base', 'base.sql')
    if not f:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)
    base_sql_file = izi.tools.misc.file_open(f)
    try:
    ...
    for i in izi.modules.get_modules():
        mod_path = izi.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = izi.modules.load_information_from_description_file(i)
