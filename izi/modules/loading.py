...
import izi
import izi.modules.db
import izi.modules.graph
import izi.modules.migration
import izi.modules.registry
import izi.tools as tools

from izi import api, SUPERUSER_ID
from izi.modules.module import adapt_version, initialize_sys_path, load_openerp_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('izi.tests')
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    def load_test(module_name, idref, mode):
        ...
        try:
            ...
            if tools.config.options['test_commit']:
                ...
                izi.registry(cr.dbname).clear_caches()
    ...
    migrations = izi.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph, 1):
        ...
        if new_install:
            py_module = sys.modules['izi.addons.%s' % (module_name,)]
        ...
        if hasattr(package, 'init') or hasattr(package, 'update') or package.state in ('to install', 'to upgrade'):
            ...
            overwrite = izi.tools.config["overwrite_existing_translations"]
            ...
            if has_demo:
                ...
                if tools.config.options['test_enable']:
                    ...
                    report.record_result(izi.modules.module.run_unit_tests(module_name, cr.dbname))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, izi.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    cr = db.cursor()
    try:
        if not izi.modules.db.is_initialized(cr):
        ...
        # izi.modules.registry.Registry.new().
        registry = izi.registry(cr.dbname)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['izi.addons.%s' % (pkg.name,)]
                ...
                registry = izi.modules.registry.Registry.new(
