...
import izi
import izi.tools as tools
import izi.release as release
from izi import SUPERUSER_ID, api
...
class AddonsHook(object):
    """ Makes modules accessible through openerp.addons.* and izi.addons.* """

    def find_module(self, name, path=None):
        if name.startswith(('izi.addons.', 'openerp.addons.'))\
                and name.count('.') == 2:
            return self
    ...
    def load_module(self, name):
        ...
        # get canonical names
        izi_name = re.sub(r'^openerp.addons.(\w+)$', r'izi.addons.\g<1>', name)
        openerp_name = re.sub(r'^izi.addons.(\w+)$', r'openerp.addons.\g<1>', izi_name)

        assert izi_name not in sys.modules
        ...
        setattr(izi.addons, addon_name, new_mod)
...
class iziHook(object):
    """ Makes izi package also available as openerp """
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'izi\g<1>', name)
...
def initialize_sys_path():
    ...
    ``import izi.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global ad_paths
    global hooked
    ...
    # add izi.addons.__path__
    for ad in __import__('izi.addons').addons.__path__:
        ad = os.path.abspath(ad)
        if ad not in ad_paths and os.path.isdir(ad):
            ad_paths.append(ad)

    if not hooked:
        sys.meta_path.insert(0, iziHook())
...
def get_module_filetree(module, dir='.'):
    ...
    files = izi.tools.osutil.listdir(path, True)
...
def load_openerp_module(module_name):
    ...
    initialize_sys_path()
    try:
        __import__('izi.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['izi.addons.' + module_name], info['post_load'])()
...
class TestStream(object):
    def __init__(self, logger_name='izi.tests'):
...
def run_unit_tests(module_name, dbname, position=runs_at_install):
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, izi.sql_db.sql_counter - t0_sql)
