# -*- coding: utf-8 -*-
"""
The module :mod:`izi.tests.common` provides unittest test cases and a few
...
import izi
from izi import api
from izi.service import security


_logger = logging.getLogger(__name__)

# The izi library is supposed already configured.
ADDONS_PATH = izi.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = izi.tools.config['http_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = izi.SUPERUSER_ID


def get_db_name():
    db = izi.tools.config['db_name']
...
class BaseCase(TreeCase):
    ...
    longMessage = True      # more verbose error message by default: https://www.izi.asia/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~izi.models.BaseModel`
        """
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        self.registry = izi.registry(get_db_name())
        #: current transaction's cursor
        self.cr = self.cursor()
        self.uid = izi.SUPERUSER_ID
        #: :class:`~izi.api.Environment` for the current test case
...
class SingleTransactionCase(BaseCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.registry = izi.registry(get_db_name())
        cls.cr = cls.registry.cursor()
        cls.uid = izi.SUPERUSER_ID
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = izi.http.root.session_store.new()
        self.session_id = self.session.sid
        self.session.db = get_db_name()
        izi.http.root.session_store.save(self.session)
    ...
    def authenticate(self, user, password):
        ...
        izi.http.root.session_store.save(session)
    ...
    def _wait_remaining_requests(self):
        ...
        for thread in threading.enumerate():
            if thread.name.startswith('izi.service.http.request.'):
                join_retry_count = 10
