...
class MetaModel(api.Meta):
    ...
    def _get_addon_name(self, full_name):
        # The (OpenERP) module name can be in the ``izi.addons`` namespace
        # or not. For instance, module ``sale`` can be imported as
        # ``izi.addons.sale`` (the right way) or ``sale`` (for backward
        # compatibility).
        module_parts = full_name.split('.')
        if len(module_parts) > 2 and module_parts[:2] == ['izi', 'addons']:
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    """ Base class for izi models.

    izi models are created by inheriting:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = izi.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or izi.fields.Datetime.now()
    ...
    def _add_fake_fields(self, fields):
        from izi.fields import Char, Integer
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~izi.fields.Integer`,
          :class:`~izi.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~izi.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~izi.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~izi.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~izi.fields.Date` and
              :class:`~izi.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`izi.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`izi.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _openerp/models/relationals/format:

          :class:`~izi.fields.One2many` and
          :class:`~izi.fields.Many2many` use a special "commands" format to
          ...
          ``(3, id, _)``
              ...
              :class:`~izi.fields.One2many`. Can not be used in
              :meth:`~.create`.
          ``(4, id, _)``
              adds an existing record of id ``id`` to the set. Can not be
              used on :class:`~izi.fields.One2many`.
          ``(5, _, _)``
    ...
    def with_env(self, env):
        ...
        :type env: :class:`~izi.api.Environment`
        """
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted izi models.

    izi models are created by inheriting from this class::
