...
from izi.exceptions import except_orm, UserError
from izi.tools import ustr, pycompat
...
class IrMailServer(models.Model):
    ...
    def connect(self, host=None, port=None, user=None, password=None, encryption=None,
        ...
        if smtp_encryption == 'ssl':
            if 'SMTP_SSL' not in smtplib.__all__:
                raise UserError(
                    _("Your izi Server does not support SMTP-over-SSL. "
    ...
    def _get_default_bounce_address(self):
        ...
        If "mail.bounce.alias" is not set it defaults to "postmaster-izi".

        If "mail.catchall.domain" is not set, return None.

        '''
        get_param = self.env['ir.config_parameter'].sudo().get_param
        postmaster = get_param('mail.bounce.alias', default='postmaster-izi')
