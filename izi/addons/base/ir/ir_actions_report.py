...
from izi.tools.misc import find_in_path
from izi.tools import config
from izi.sql_db import TestCursor
...
# here to init the T1 fonts cache at the start-up of izi so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at izi start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start izi with at least two workers to print a pdf version of the reports.')
