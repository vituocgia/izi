...
import izi
from izi import api, http, models, tools, SUPERUSER_ID
from izi.exceptions import AccessDenied, AccessError
from izi.http import request, STATIC_CACHE, content_disposition
from izi.tools import pycompat, consteq
from izi.tools.mimetypes import guess_mimetype
from izi.modules.module import get_resource_path, get_module_path
...
class IrHttp(models.AbstractModel):
    ...
    def routing_map(cls):
        if not hasattr(cls, '_routing_map'):
            ...
            if tools.config['test_enable'] and izi.modules.module.current_test:
                installed.add(izi.modules.module.current_test)
            mods = [''] + izi.conf.server_wide_modules + sorted(installed)
