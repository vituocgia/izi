...
from izi.exceptions import MissingError, UserError, ValidationError, AccessError
from izi.tools.safe_eval import safe_eval, test_python_expr
...
class IrActionsServer(models.Model):
    ...
    Since izi 8.0 a button 'Create Menu Action' button is available on the
...
#  - env: izi Environment on which the action is triggered
#  - model: izi Model of the record on which the action is triggered; is a void recordset
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    def _get_eval_context(self, action=None):
        ...
        eval_context.update({
            ...
            'model': model,
            # Exceptions
            'Warning': izi.exceptions.Warning,
            # record
            'record': record,
            'records': records,
            # helpers
