...
import izi
from izi import api, fields, models, modules, tools, _
from izi.exceptions import AccessDenied, UserError
from izi.tools.parse_version import parse_version
from izi.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    license = fields.Selection([
        ...
        ('OEEL-1', 'izi Enterprise Edition License v1.0'),
        ('OPL-1', 'izi Proprietary License v1.0'),
    ...
    def install_from_urls(self, urls):
        ...
        OPENERP = izi.release.product_name.lower()
        ...
        try:
            ...
            if urls.get(OPENERP):
                ...
                # copy all modules in the SERVER/izi/addons directory to the new "izi" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, OPENERP, 'addons', d)    # XXX 'izi' subdirectory ?
                ...
                _logger.info('Copy downloaded module `izi` to `%s`', server_dir)
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.izi.asia/apps')
