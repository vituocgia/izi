...
from izi.exceptions import AccessDenied, AccessError, UserError, ValidationError
from izi.http import request
from izi.osv import expression
from izi.service.db import check_super
from izi.tools import partition, pycompat
...
class Users(models.Model):
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by izi (updates, module installation, ...)'))
        db = self._cr.dbname
        for id in self.ids:
            self.__uid_cache[db].pop(id, None)
        self._invalidate_session_cache()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: izi.exceptions.AccessDenied when old password is wrong
        :raise: izi.exceptions.UserError when new password is not set or empty
        """
