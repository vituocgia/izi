...
class Field(MetaField('DummyField', (object,), {})):
    """ The field descriptor contains the field definition, and manages accesses
        ...
        recordset. The decorator :meth:`izi.api.depends` must be applied on
        ...
        is extended (see :class:`~izi.models.Model`), one can also extend
    ...
    def cache_key(self, record):
        ...
        """
        env = record.env
        # IMPORTANT: izi.api.Cache.get_records() depends on the fact that the
...
class Monetary(Field):
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if validate and record[self.currency_field]:
            # FIXME @rco-izi: currency may not be already initialized if it is
