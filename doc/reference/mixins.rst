...
izi implements some useful classes and mixins that make it easy for you to add
...
``_mail_post_access`` - :class:`~izi.models.Model`  attribute
...
database without needing to connect to izi directly.
...
catchall domain will be sufficient since all routing will be done inside izi.
...
.. note:: Aliases can also be created manually and supported by a simple
    :class:`~izi.fields.Many2one` field. This guide assumes you wish a 
...
* ``campaign_id``: :class:`~izi.fields.Many2one` field to a ``utm.campaign``
  object (i.e. Christmas_Special, Fall_Collection, etc.)
* ``source_id``: :class:`~izi.fields.Many2one` field to a ``utm.source`` 
  object (i.e. Search Engine, mailing list, etc.)
* ``medium_id``: :class:`~izi.fields.Many2one` field to a ``utm.medium`` 
...
(i.e. http://www.izi.asia/?campaign_id=mixin_talk&source_id=www.izi.asia&medium_id=website),
    ...
    class MyModel(models.Models):
        ...
        def tracking_fields(self):
            ...
            # ("URL_PARAMETER", "FIELD_NAME_MIXIN", "NAME_IN_COOKIES")
                ('my_field', 'my_field', 'izi_utm_my_field')
            ])
            return result

This will tell the system to create a cookie named *izi_utm_my_field* with the
...
* ``helpdesk.ticket`` in the Helpdesk (*helpdesk* - izi Enterprise only) Application
...
* ``website_published``: :class:`~izi.fields.Boolean` field which represents
  the status of the publication
* ``website_url``: :class:`~izi.fields.Char` field which represents
...
* ``website_meta_title``: :class:`~izi.fields.Char` field that allow you to set
  an additional title to your page
* ``website_meta_description``: :class:`~izi.fields.Char` field that contains a
  short description of the page (sometimes used in search engines results)
* ``website_meta_keywords``: :class:`~izi.fields.Char` field that contains some
