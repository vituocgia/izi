...
This document presents the izi Javascript framework. This
...
.. note:: An html version of all docstrings in izi is available at:
...
- the *website*: this is the public part of izi.  It allows an unidentified
...
It means that while a user is working on izi, the web client class (and the
...
Managing assets in izi is not as straightforward as it is in some other apps.
...
a collection of files (javascript, css, less). In izi, the most important
...
Main bundles
------------
When the izi server is started, it checks the timestamp of each file in a bundle,
...
- *web.assets_common*: this bundle contains most assets which are common to the
  ...
  lower level building blocks for the izi framework.  Note that it contains the
  *boot.js* file, which defines the izi module system.
...
.. note ::
    ...
    izi web client.  This means that the files are transferred through the network
...
sure they are loaded in the correct order.  In order to do that, izi has defined
a small module system (located in the file *addons/web/static/src/js/boot.js*,
which needs to be loaded first).

The izi module system, inspired by AMD, works by defining the function *define*
on the global izi object. We then define each javascript module by calling that
function.  In the izi framework, a module is a piece of code that will be executed
...
.. code-block:: javascript
    ...
    // in file a.js
    izi.define('module.A', function (require) {
        "use strict";
    ...
    // in file b.js
    izi.define('module.B', function (require) {
        "use strict";
...
.. code-block:: javascript
    ...
    izi.define('module.Something', ['module.A', 'module.B'], function (require) {
        "use strict";
...
The *izi.define* method is given three arguments:

- *moduleName*: the name of the javascript module.  It should be a unique string.
  The convention is to have the name of the izi addon followed by a specific
...
.. code-block:: javascript
    ...
    izi.define('module.Something', ['web.ajax'], function (require) {
        "use strict";
...
izi was developped before ECMAScript 6 classes were available.  In Ecmascript 5,
...
For these reasons, izi decided to use its own class system, inspired by John
...
The izi Class system does not support multiple inheritance, but for those cases
...
the way izi is structured, it is sometimes necessary in one addon to modify
...
* Global selectors should be avoided. Because a component may be used several
  times in a single page (an example in izi is dashboards), queries should be
  ...
  lead to unintended or incorrect behavior.  izi Web's
...
There are currently two event systems supported by izi: a simple system which
...
- *trigger_up*: this is the method that will create a small *iziEvent* and
...
The iziEvent class is very simple.  It has three public attributes: *target*
...
A common need in the izi ecosystem is to extend/change the behaviour of the
...
There are typically two usecases when working on izi: one may need to call a
...
In 2017, izi replaced the previous view code with a new architecture.  The
