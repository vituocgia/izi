...
``views``
    ...
    :meth:`~izi.models.Model.fields_view_get`). The first type of the
...
Allow opening a URL (website/web page) via an izi action. Can be customized
via two fields:
...
will replace the current content section by the izi home page.
...
``model_id``
    izi model linked to the action, made available in
...
Creates a new record, from scratch (via :meth:`~izi.models.Model.create`)
or by copying an existing record (via :meth:`~izi.models.Model.copy`)
...
``fields_lines``
    ...
    :class:`~izi.fields.One2many` with the fields:
...
``crud_model_id``
    ...
    ``new_other``
``ref_object``
    :class:`~izi.fields.Reference` to an arbitrary record to copy, used if
...
``groups_id``
    ...
    the current report
``paperformat_id``
    :class:`~izi.fields.Many2one` field to the paper format you wish to
