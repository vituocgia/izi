...
.. versionadded:: 8.0
    ...
    This page documents the New API added in izi 8.0 which should be the
...
Accessing a relational field (:class:`~izi.fields.Many2one`,
:class:`~izi.fields.One2many`, :class:`~izi.fields.Many2many`)
...
.. danger::
    ...
    (to the same value), use :meth:`~izi.models.Model.write`::
...
izi maintains a cache for the fields of the records, so that not every field
...
To avoid reading one field on one record at a time, izi *prefetches* records
...
:meth:`~izi.models.Model.filtered`
...
The :class:`~izi.api.Environment` stores various contextual data used by
...
using :attr:`~izi.models.Model.env` and gives access to the current user
(:attr:`~izi.api.Environment.user`), the cursor
(:attr:`~izi.api.Environment.cr`) or the context
(:attr:`~izi.api.Environment.context`)::
...
:meth:`~izi.models.Model.with_context`
...
:meth:`~izi.models.Model.with_env`
...
:meth:`~izi.models.Model.search`
   ...
   .. tip:: to just check if any record matches a domain, or count the number
             ...
             :meth:`~izi.models.Model.search_count`
:meth:`~izi.models.Model.create`
...
:meth:`~izi.models.Model.ensure_one`
...
* computed fields are automatically checked and recomputed, they do not need
  an ``onchange``
* for non-computed fields, the :func:`~izi.api.onchange` decorator is used
...
.. warning::
    ...
    itself via onchange. This is a webclient limitation - see `#2693 <https://github.com/izi/izi/issues/2693>`_.

Low-level SQL
-------------

The :attr:`~izi.api.Environment.cr` attribute on environments is the
...
Because models use the same cursor and the :class:`~izi.api.Environment`
...
:meth:`~izi.api.Environment.invalidate_all` method of the
:class:`~izi.api.Environment` object.
...
izi is currently transitioning from an older (less regular) API, it can be
...
* values of the :class:`~izi.api.Environment` (cursor, user id and
  context) are passed explicitly to methods instead
* record data (:attr:`~izi.models.Model.ids`) are passed explicitly to
...
.. currentmodule:: izi.models

.. autoclass:: izi.models.Model
    ...
    .. attribute:: _auto
     ...
     .. tip:: To create a model without any table, inherit
              from ``izi.models.AbstractModel``

    .. attribute:: _table

        Name of the table backing the model created when
        :attr:`~izi.models.Model._auto`, automatically generated by
        default.
    ...
    .. attribute:: _inherits
        ...
        the fields of the :attr:`~izi.models.Model._inherits`-ed model but
        ...
        .. warning::
            ...
            :attr:`~izi.models.Model._inherits`-ed
    ...
    .. attribute:: id
        ...
        Identifier :class:`field <izi.fields.Field>`
    ...
    .. attribute:: sequence
        ...
        :type: :class:`~izi.fields.Integer`

    .. attribute:: state

        lifecycle stages of the object, used by the ``states`` attribute on
        :class:`fields <izi.fields.Field>`

        :type: :class:`~izi.fields.Selection`
    ...
    .. attribute:: parent_id
        ...
        :type: :class:`~izi.fields.Many2one`
...
.. autodoc documents descriptors as attributes, even for the *definition* of
   descriptors. As a result automodule:: izi.fields lists all the field
...
.. autoclass:: izi.fields.Field

.. autoclass:: izi.fields.Char
    :show-inheritance:

.. autoclass:: izi.fields.Boolean
    :show-inheritance:

.. autoclass:: izi.fields.Integer
    :show-inheritance:

.. autoclass:: izi.fields.Float
    :show-inheritance:

.. autoclass:: izi.fields.Text
    :show-inheritance:

.. autoclass:: izi.fields.Selection
    :show-inheritance:

.. autoclass:: izi.fields.Html
    :show-inheritance:

.. autoclass:: izi.fields.Date
    :show-inheritance:
    :members: today, context_today, from_string, to_string

.. autoclass:: izi.fields.Datetime
    :show-inheritance:
...
.. autoclass:: izi.fields.Many2one
    :show-inheritance:

.. autoclass:: izi.fields.One2many
    :show-inheritance:

.. autoclass:: izi.fields.Many2many
    :show-inheritance:

.. autoclass:: izi.fields.Reference
    :show-inheritance:
...
izi provides three different mechanisms to extend models in a modular way:
...
When using the :attr:`~izi.models.Model._inherit` and
:attr:`~izi.models.Model._name` attributes together, izi creates a new
model using the existing one (provided via
:attr:`~izi.models.Model._inherit`) as a base. The new model gets all the
fields, methods and meta-information (defaults & al) from its base.

.. literalinclude:: ../../izi/addons/test_documentation_examples/inheritance.py
    :language: python
    :lines: 5-

and using them:

.. literalinclude:: ../../izi/addons/test_documentation_examples/tests/test_inheritance.py
    :language: python
    :lines: 10,11,14,19
...
When using :attr:`~izi.models.Model._inherit` but leaving out
:attr:`~izi.models.Model._name`, the new model replaces the existing one,
...
.. literalinclude:: ../../izi/addons/test_documentation_examples/extension.py
    :language: python
    :lines: 5-

.. literalinclude:: ../../izi/addons/test_documentation_examples/tests/test_extension.py
    :language: python
    :lines: 8,13
...
at runtime) but less power: using the :attr:`~izi.models.Model._inherits`
...
:class:`~izi.fields.Reference` fields automatically set up on the parent
model:

.. literalinclude:: ../../izi/addons/test_documentation_examples/delegation.py
    :language: python
    :lines: 5-

.. literalinclude:: ../../izi/addons/test_documentation_examples/tests/test_delegation.py
    :language: python
    :lines: 11-14,23,28
...
``field_name`` (``str``)
    ...
    a :class:`~izi.fields.Many2one` using dot-notation e.g. ``'street'``
...
``operator`` (``str``)
    ...
    ``child_of``
        ...
        :attr:`~izi.models.Model._parent_name`).
...
* :meth:`~izi.models.Model.search` returns a recordset, no point in e.g.
  browsing its result
* ``fields.related`` and ``fields.function`` are replaced by using a normal
  field type with either a ``related=`` or a ``compute=`` parameter
* :func:`~izi.api.depends` on ``compute=`` methods **must be complete**,
...
* the decorators :func:`~izi.api.model` and :func:`~izi.api.multi` are
  for bridging *when calling from the old API context*, for internal or pure
  new-api (e.g. compute) they are useless
* remove :attr:`~izi.models.Model._default`, replace by ``default=``
...
* double check that all fields and methods have different names, there is no
  warning in case of collision (because Python handles it before izi sees
  anything)
* the normal new-api import is ``from izi import fields, models``. If
  compatibility decorators are necessary, use ``from izi import api,
  fields, models``
* avoid the :func:`~izi.api.one` decorator, it probably does not do what
  you expect
* remove explicit definition of :attr:`~izi.models.Model.create_uid`,
  :attr:`~izi.models.Model.create_date`,
  :attr:`~izi.models.Model.write_uid` and
  :attr:`~izi.models.Model.write_date` fields: they are now created as
...
* when straight conversion is impossible (semantics can not be bridged) or the
  ...
  implementations for the same method name using :func:`~izi.api.v7` and
  :func:`~izi.api.v8`. The method should first be defined using the
  old-API style and decorated with :func:`~izi.api.v7`, it should then be
  re-defined using the exact same name but the new-API style and decorated
  with :func:`~izi.api.v8`. Calls from an old-API context will be
...
* uses of :attr:`~izi.models.Model._columns` or
  :attr:`~izi.models.Model._all_columns` should be replaced by
  :attr:`~izi.models.Model._fields`, which provides access to instances of
  new-style :class:`izi.fields.Field` instances (rather than old-style
  :class:`izi.osv.fields._column`).

  Non-stored computed fields created using the new API style are *not*
  available in :attr:`~izi.models.Model._columns` and can only be
  inspected through :attr:`~izi.models.Model._fields`
* reassigning ``self`` in a method is probably unnecessary and may break
  translation introspection
* :class:`~izi.api.Environment` objects rely on some threadlocal state,
  which has to be set up before using them. It is necessary to do so using the
  :meth:`izi.api.Environment.manage` context manager when trying to use
  the new API in contexts where it hasn't been set up yet, such as new threads
  or a Python interactive environment::

    >>> from izi import api, modules
...
:class:`~izi.api.Environment` (for :attr:`~izi.api.Environment.cr`,
:attr:`~izi.api.Environment.user` and
:attr:`~izi.api.Environment.context`) or the current recordset (for ``id``
and ``ids``).
...
* disabling it entirely, by decorating a method with
  :func:`~izi.api.noguess` there will be no bridging and methods will be
...
* defining the bridge explicitly, this is mostly for methods which are matched
  ...
  (e.g. :func:`~izi.api.cr_uid_context`) which also passes the current
  context *by keyword*.
* dual implementations using :func:`~izi.api.v7` and
  :func:`~izi.api.v8` will be ignored as they provide their own "bridging"
