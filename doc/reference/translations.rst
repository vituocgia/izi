...
automatically loaded by izi when the corresponding language is installed (via
...
izi automatically exports translatable strings from "data"-type content:
...
* QWeb templates (both server-side and client-side), all text nodes are
  ...
  exported
* for :class:`~izi.fields.Field`, unless their model is marked with
  ``_translate = False``:
...
* help/error messages of :attr:`~izi.models.Model._constraints` and
  :attr:`~izi.models.Model._sql_constraints` are exported

Explicit exports
================

When it comes to more "imperative" situations in Python code or Javascript
code, izi cannot automatically export translatable terms so they
must be marked explicitly for export. This is done by wrapping a literal
string in a function call.

In Python, the wrapping function is :func:`izi._`::

    title = _("Bank Accounts")

In JavaScript, the wrapping function is generally :js:func:`izi.web._t`:
