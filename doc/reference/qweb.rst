...
QWeb is the primary templating_ engine used by izi\ [#othertemplates]_. It
...
:meth:`izi.http.HttpRequest.render`:
...
This automatically creates a :class:`~izi.http.Response` object which can
...
.. py:method:: render(cr, uid, id[, values][, engine='ir.qweb][, context])
    ...
    ``request``
        the current :class:`~izi.http.WebRequest` object, if any
    ``debug``
    ...
    :param str engine: name of the izi model to use for rendering, can be
...
.. .. automodule:: izi.addons.base.ir.ir_qweb
...
.. [#othertemplates] although it uses a few others, either for historical
                     ...
                     use case. izi 9.0 still depends on Jinja_ and Mako_.
