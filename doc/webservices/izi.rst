...
izi is usually extended internally via modules, but many of its features and
all of its data are also available from the outside for external analysis or
integration with various tools. Part of the :ref:`reference/orm/model` API is
easily available over XML-RPC_ and accessible from a variety of languages.

.. izi XML-RPC idiosyncracies:
   * uses multiple endpoint and a nested call syntax instead of a
     "hierarchical" server structure (e.g. ``izi.res.partner.read()``)
...
.. only:: html
    ...
    .. rst-class:: setupcode hidden
        ...
        .. code-block:: python
            ...
            import xmlrpclib
            info = xmlrpclib.ServerProxy('https://demo.izi.asia/start').start()
        ...
        .. code-block:: ruby
            ...
            info = XMLRPC::Client.new2('https://demo.izi.asia/start').call('start')
            url, db, username, password = \
                info['host'], info['database'], info['user'], info['password']
        ...
        .. code-block:: php
            ...
            $info = ripcord::client('https://demo.izi.asia/start')->start();
        ...
        .. code-block:: java
            ...
            start_config.setServerURL(new URL("https://demo.izi.asia/start"));
...
If you already have an izi server installed, you can just use its
parameters

.. warning::

    For izi Online instances (<domain>.izi.asia), users are created without a
    *local* password (as a person you are logged in via the izi Online
    authentication system, not by the instance itself). To use XML-RPC on izi
    ...
    *https://mycompany.izi.asia*), the *database name* is the name of the
...
To make exploration simpler, you can also ask https://demo.izi.asia for a test
database:
...
izi requires users of the API to be authenticated before they can query most 
data.
...
The second endpoint is ``xmlrpc/2/object``, is used to call methods of izi
...
Records can be listed and filtered via :meth:`~izi.models.Model.search`.

:meth:`~izi.models.Model.search` takes a mandatory
...
:meth:`~izi.models.Model.search_count` can be used to retrieve
only the number of records matching the query. It takes the same
:ref:`domain <reference/orm/domains>` filter as
:meth:`~izi.models.Model.search` and no other parameter.
...
Record data is accessible via the :meth:`~izi.models.Model.read` method,
which takes a list of ids (as returned by
:meth:`~izi.models.Model.search`) and optionally a list of fields to
...
:meth:`~izi.models.Model.fields_get` can be used to inspect
...
Because it is a very common task, izi provides a
:meth:`~izi.models.Model.search_read` shortcut which as its name suggests is
equivalent to a :meth:`~izi.models.Model.search` followed by a
:meth:`~izi.models.Model.read`, but avoids having to perform two requests
and keep ids around.

Its arguments are similar to :meth:`~izi.models.Model.search`'s, but it
can also take a list of ``fields`` (like :meth:`~izi.models.Model.read`,
...
Records of a model are created using :meth:`~izi.models.Model.create`. The
method will create a single record and return its database identifier.

:meth:`~izi.models.Model.create` takes a mapping of fields to values, used
...
.. warning::
    ...
    :class:`~izi.fields.Integer`, string for :class:`~izi.fields.Char`
    or :class:`~izi.fields.Text`),

    * :class:`~izi.fields.Date`, :class:`~izi.fields.Datetime` and
      :class:`~izi.fields.Binary` fields use string values
    * :class:`~izi.fields.One2many` and :class:`~izi.fields.Many2many`
      use a special command protocol detailed in :meth:`the documentation to
      the write method <izi.models.Model.write>`.

Update records
--------------

Records can be updated using :meth:`~izi.models.Model.write`, it takes
a list of records to update and a mapping of updated fields to values similar
to :meth:`~izi.models.Model.create`.
...
While we previously used :meth:`~izi.models.Model.fields_get` to query a
model and have been using an arbitrary model from the start, izi stores
...
Provides information about izi models via its various fields
...
``field_id``
    list of the model's fields through a :class:`~izi.fields.One2many` to
    :ref:`reference/webservice/inspection/fields`
``view_ids``
    :class:`~izi.fields.One2many` to the :ref:`reference/views` defined
    for the model
``access_ids``
    :class:`~izi.fields.One2many` relation to the
...
Provides information about the fields of izi models and allows adding
custom fields without using Python code

``model_id``
    :class:`~izi.fields.Many2one` to
...
``groups``
    ...
    :class:`~izi.fields.Many2many` to ``res.groups``
