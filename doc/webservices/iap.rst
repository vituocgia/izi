...
In-App Purchase (IAP) allow providers of ongoing services through izi apps to
be compensated for ongoing service use rather than — and possibly instead of
— a sole initial purchase.

In that context, izi acts mostly as a *broker* between a client and an izi
App Developer:

* users purchase service tokens from izi
* service providers draw tokens from the user's izi account when service
  is requested

.. attention::

    This document is intended for *service providers* and presents the latter,
    which can be done either via direct JSON-RPC2_ or if you are using izi
...
.. figure:: images/players.png
    ...
    * The Client installed your izi App, and from there will request services
    * izi brokers crediting, the Client adds credit to their account, and you
    ...
    * The External Service is an optional player: *you* can either provide a
      ...
      bridge/translator between an izi system and the actual service
...
.. figure:: images/credits.jpg
    ...
    * for a postage service: 1 credit = 1 post stamp.
    ...
    that the clients can buy on https://iap.izi.asia (see :ref:`Packages <iap-packages>`).
...
.. figure:: images/normal.png
    ...
    2. the Service Provider asks izi if there are enough credits for the
    ...
    4. the Service Provider goes back to izi to capture (if the service could
...
.. figure:: images/no-credit.png
    ...
    2. the Service Provider asks izi if there are enough credits on the
       Client's account and gets a negative reply
    3. this is signaled back to the Client
    4. who is redirected to their izi account to credit it and re-try
...
go to your *Portal Account* on the IAP endpoint (https://iap.izi.asia for
production, https://iap-sandbox.izi.asia for testing, the endpoints are
...
* :samp:`name` - :class:`ServiceName`: this will identify your service in the
  client's :ref:`app <iap-izi-app>` communicates directly with IAP.
...
.. note:: 
    ...
    izi takes a 25% commission on all package sales. Adjust your selling price accordingly.
...
The second step is to develop an `izi App`_ which clients can install in their
izi instance and through which they can *request* services you will provide.
Our app will just add a button to the Partners form which lets a user request
burning some CPU time on the server.

First, we'll create an *izi module* depending on ``iap``. IAP is a standard
...
:func:`~izi.addons.iap.jsonrpc` helper to call a JSON-RPC2_ endpoint on an
other izi instance and transparently re-raise relevant izi exceptions
(:class:`~izi.addons.iap.models.iap.InsufficientCreditError`,
:class:`izi.exceptions.AccessError` and :class:`izi.exceptions.UserError`).
...
.. note::
    ...
    :class:`~izi.addons.iap.models.iap.InsufficientCreditError` coming from the action
    and prompts the user to add credits to their account.

    :func:`~izi.addons.iap.jsonrpc` takes care of re-raising
    :class:`~izi.addons.iap.models.iap.InsufficientCreditError` for you.

.. danger::

    If you are not using :func:`~izi.addons.iap.jsonrpc` you *must* be
    careful to re-raise
    :class:`~izi.addons.iap.models.iap.InsufficientCreditError` in your handler
...
for JSON-RPC2_ calls (:func:`~izi.addons.iap.jsonrpc`) and a service helper
for transactions (:class:`~izi.addons.iap.models.iap.charge`) we will also be
implementing the service side as an izi module:

.. patch::

Since the query from the client comes as JSON-RPC2_ we will need the
corresponding controller which can call :class:`~izi.addons.iap.models.iap.charge` and
perform the service within:

.. patch::

.. todo:: for the actual IAP will the "portal" page be on izi.asia or iap.izi.asia?

.. todo:: "My Account" > "Your InApp Services"?


The :class:`~izi.addons.iap.models.iap.charge` helper will:
...
.. danger::
    ...
    By default, :class:`~izi.addons.iap.models.iap.charge` contacts the *production*
    IAP endpoint, https://iap.izi.asia. While developing and testing your
    service you may want to point it towards the *development* IAP endpoint
    https://iap-sandbox.izi.asia.

    To do so, set the ``iap.endpoint`` config parameter in your service
    izi: in debug/developer mode, :menuselection:`Setting --> Technical -->
    Parameters --> System Parameters`, just define an entry for the key
    ``iap.endpoint`` if none already exists).

The :class:`~izi.addons.iap.models.iap.charge` helper has two additional optional
...
* The IAP transaction API does not require using izi when implementing your
...
.. function:: /iap/1/authorize
    ...
    *and creates a hold (pending transaction) on that amount*.
    ...
    :raises: :class:`~izi.exceptions.AccessError` if the service token is invalid
    :raises: :class:`~izi.addons.iap.models.iap.InsufficientCreditError` if the account does
    :raises: ``TypeError`` if the ``credit`` value is not an integer

.. code-block:: python

    r = requests.post(izi + '/iap/1/authorize', json={
        'jsonrpc': '2.0',
        'id': None,
        'method': 'call',
        'params': {
...
.. function:: /iap/1/capture
    ...
    :raises: :class:`~izi.exceptions.AccessError`

.. code-block:: python
  :emphasize-lines: 8
   
    r2 = requests.post(izi + '/iap/1/capture', json={
        'jsonrpc': '2.0',
        'id': None,
        'method': 'call',
        'params': {
...
.. code-block:: python
    ...
    r2 = requests.post(izi + '/iap/1/cancel', json={
        'jsonrpc': '2.0',
        'id': None,
        'method': 'call',
        'params': {
...
.. class:: ServiceName
    ...
    String identifying your service on https://iap.izi.asia (production) as well
...
.. exception:: izi.addons.iap.models.iap.InsufficientCreditError
...
.. exception:: izi.exceptions.AccessError

    Raised by:

    * any operation to which a service token is required, if the service token is invalid.
    * any failure in an inter-server call. (typically, in :func:`~izi.addons.iap.jsonrpc`)

.. exception:: izi.exceptions.UserError
...
For convenience, if you are implementing your service using izi the ``iap``
...
.. class:: izi.addons.iap.models.iap.charge(env, key, account_token, credit[, description, credit_template])
    ...
    * otherwise cancels it
    ...
    :param izi.api.Environment env: used to retrieve the ``iap.endpoint``
                                     configuration key
    :param ServiceKey key:
...
.. _izi App: https://www.izi.asia/apps
