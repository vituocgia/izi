:banner: banners/deploying_izi.jpg

==============
Deploying izi
==============

This document describes basic steps to set up izi in production or on an
...
izi is a multi-tenant system: a single izi system may run and serve a number
...
bound to a database: izi needs to know which database should be used to load
...
accessible izi needs a rule to know which one it should use.

That is one of the purposes of :option:`--db-filter <izi-bin --db-filter>`:
...
.. note::
  Setting a proper :option:`--db-filter <izi-bin --db-filter>` is an important part
...
UNIX socket is fine if you want izi and PostgreSQL to execute on the same
machine, and is the default when no host is provided, but if you want izi and
...
* Only accept loopback connections and `use an SSH tunnel`_ between the
  machine on which izi runs and the one on which PostgreSQL runs, then
  configure izi to connect to its end of the tunnel
* Accept connections to the machine on which izi is installed, possibly
  over ssl (see `PostgreSQL connection settings`_ for details), then configure
  izi to connect over the network
...
Out of the box, izi connects to a local postgres over UNIX socket via port
...
create a new user (``izi``) and set it as the database user.
...
SSL Between izi and PostgreSQL
-------------------------------

Since izi 11.0, you can enforce ssl connection between izi and PostgreSQL.
in izi the db_sslmode control the ssl security of the connection 
...
izi includes built-in HTTP servers, using either multithreading or
multiprocessing.
...
* Multiprocessing is enabled by configuring :option:`a non-zero number of
  worker processes <izi-bin --workers>`, the number of workers should be based
...
listening on :option:`the longpolling port <izi-bin --longpolling-port>` but
the client will not connect to it.

Instead you must have a proxy redirecting requests whose URL starts with
``/longpolling/`` to the longpolling port. Other request should be proxied to
the :option:`normal HTTP port <izi-bin --http-port>`

To achieve such a thing, you'll need to deploy a reverse proxy in front of izi,
like nginx or apache. When doing so, you'll need to forward some more http Headers
to izi, and activate the proxy_mode in izi configuration to have izi read those
headers.
...
* RAM = 9 * ((0.8*150) + (0.2*1024)) ~= 3Go RAM for izi
...
Whether it's accessed via website/web client or web service, izi transmits
authentication information in cleartext. This means a secure deployment of
izi must use HTTPS\ [#switching]_. SSL termination can be implemented via
just about any SSL termination proxy, but requires the following setup:

* Enable izi's :option:`proxy mode <izi-bin --proxy-mode>`. This should only be enabled when izi is behind a reverse proxy
...
in ``/etc/nginx/sites-enabled/izi.conf`` set:
...
.. code-block:: nginx
  ...
  server {
   ...
   # Add Headers for izi proxy mode
   ...
   # log
   access_log /var/log/nginx/izi.access.log;
   error_log /var/log/nginx/izi.error.log;

   # Redirect longpoll requests to izi longpolling port
   location /longpolling {
   proxy_pass http://izichat;
   }

   # Redirect requests to izi backend server
   location / {
     proxy_redirect off;
     proxy_pass http://izi;
   }
...
izi as a WSGI Application
==========================

It is also possible to mount izi as a standard WSGI_ application. izi
provides the base for a WSGI launcher script as ``izi-wsgi.example.py``. That
script should be customized (possibly after copying it from the setup directory) to correctly set the
configuration directly in :mod:`izi.tools.config` rather than through the
command-line or a configuration file.

However the WSGI server will only expose the main HTTP endpoint for the web
client, website and webservice API. Because izi does not control the creation
of workers anymore it can not setup cron or livechat workers

Cron Workers
------------

To run cron jobs for an izi deployment as a WSGI application requires

* A classical izi (run via ``izi-bin``)
...
* Which should not be exposed to the network. To ensure cron runners are not
  ...
  entirely with :option:`izi-bin --no-http` or setting ``http_enable = False``
...
* Deploy a threaded version of izi (instread of a process-based preforking
  one) and redirect only requests to URLs starting with ``/longpolling/`` to
  that izi, this is the simplest and the longpolling URL can double up as
  the cron instance.
* Deploy an evented izi via ``izi-gevent`` and proxy requests starting
  with ``/longpolling/`` to
  :option:`the longpolling port <izi-bin --longpolling-port>`.

Serving Static Files
====================

For development convenience, izi directly serves all static files in its
...
izi static files live in each module's ``static/`` folder, so static files
...
- Use appropriate database filters ( :option:`--db-filter <izi-bin --db-filter>`)
  to restrict the visibility of your databases according to the hostname.
  See :ref:`db_filter`.
  You may also use :option:`-d <izi-bin -d>` to provide your own (comma-separated)
...
- Once your ``db_name`` and ``db_filter`` are configured and only match a single database
  ...
  (this is also exposed as the :option:`--no-database-list <izi-bin --no-database-list>`
  command-line option)

- Make sure the PostgreSQL user (:option:`--db_user <izi-bin --db_user>`) is *not* a super-user,
...
- Keep installations updated by regularly installing the latest builds,
  ...
  https://www.izi.asia/page/download or http://nightly.izi.asia
...
- Run izi behind a web server providing HTTPS termination with a valid SSL certificate,
  ...
  and then enable the :option:`proxy mode <izi-bin --proxy-mode>` option.
...
:ref:`setup/deploy/izi` mentioned ``admin_passwd`` in passing.
...
izi is supported by multiple browsers for each of its versions. No 
distinction is made according to the browser version in order to be
up-to-date. izi is supported on the current browser version. The list 
...
.. [#different-machines]
    to have multiple izi installations use the same PostgreSQL database,
...
.. _POSBox: https://www.izi.asia/page/point-of-sale-hardware#part_2
