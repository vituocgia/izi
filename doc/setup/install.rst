:banner: banners/installing_izi.jpg
...
There are mutliple ways to install izi, or not install it at all, depending
on the intended use case.

This documents attempts to describe most of the installation options.

:ref:`setup/install/online`
    The easiest way to use izi in production or to try it.

:ref:`setup/install/packaged`
    Suitable for testing izi, developing modules and can be used for
    long-term production use with additional deployment and maintenance work.

:ref:`setup/install/source`
    Provides greater flexibility:  e.g. allow multiple running izi versions on
...
There are two different Editions_ of izi: the Community and Enterprise versions.
...
To simply get a quick idea of izi, demo_ instances are available. They are
...
Trivial to start with, fully managed and migrated by izi S.A., izi's SaaS_
provides private instances and starts out free. It can be used to discover and
test izi and do non-code customizations (i.e. incompatible with custom modules
or the izi Apps Store) without having to install it locally.

Can be used for both testing izi and long-term production use.
...
izi provides packaged installers for Windows, deb-based distributions
...
izi will automatically be started at the end of the installation.
...
izi 11.0 'deb' package currently supports `Debian Stretch`_, `Ubuntu Xenial`_,
`Ubuntu Zesty`_ and `Ubuntu Artful`_.

Prepare
^^^^^^^

izi needs a `PostgreSQL`_ server to run properly. The default configuration for
the izi 'deb' package is to use the PostgreSQL server on the same host as your
izi instance. Execute the following command as root in order to install
PostgreSQL server :
...
izi S.A. provides a repository that can be used with  Debian and Ubuntu
distributions. It can be used to install izi.asiamunity Edition by executing the
following commands as root:

.. code-block:: console

    # wget -O - https://nightly.izi.asia/izi.key | apt-key add -
    # echo "deb http://nightly.izi.asia/11.0/nightly/deb/ ./" >> /etc/apt/sources.list.d/izi.list
    # apt-get update && apt-get install izi
...
This will install izi as a service, create the necessary PostgreSQL_ user
...
.. warning:: Debian 9 and Ubuntu do not provide a package for the python module
             num2words.
             Textual amounts will not be rendered by izi and this could cause
...
izi 11.0 'rpm' package supports Fedora 26.
As of 2017, CentOS does not have the minimum Python requirements (3.5) for
izi 11.0.

Prepare
^^^^^^^
izi needs a `PostgreSQL`_ server to run properly. Assuming that the 'sudo'
...
izi S.A. provides a repository that can be used with the Fedora distibutions.
It can be used to install izi.asiamunity Edition by executing the following
commands:

.. code-block:: console

    $ sudo dnf config-manager --add-repo=https://nightly.izi.asia/11.0/nightly/rpm/izi.repo
    $ sudo dnf install -y izi
    $ sudo systemctl enable izi
    $ sudo systemctl start izi
...
.. code-block:: console
    ...
    $ sudo dnf localinstall izi_11.0.latest.noarch.rpm
    $ sudo systemctl enable izi
    $ sudo systemctl start izi
...
The source "installation" really is about not installing izi, and running
it directly from source instead.

This can be more convenient for module developers as the izi source is
...
It also makes starting and stopping izi more flexible and explicit than the
...
easily keeping (and running) multiple versions of izi side-by-side.
...
* PostgreSQL, to use a local database
  ...
  - on Windows, use `PostgreSQL for windows`_ then
    ...
    - create a postgres user with a password using the pg admin gui: open
      ...
      usename in the :guilabel:`Role Name` field (e.g. ``izi``), then open
      the :guilabel:`Definition` tab and enter the password (e.g. ``izi``),
      then click :guilabel:`OK`.

      The user and password must be passed to izi using either the
      :option:`-w <izi-bin -w>` and :option:`-r <izi-bin -r>` options or
...
* Python dependencies listed in the :file:`requirements.txt` file.
  ...
  - on Windows you need to install some of the dependencies manually, tweak the
    ...
    command from a cmd.exe prompt (replace ``\YouriziPath`` by the actual
    path where you downloaded izi):

    .. code-block:: doscon

        C:\> cd \YouriziPath
        C:\YouriziPath> C:\Python35\Scripts\pip.exe install -r requirements.txt
...
There are two ways to obtain the izi source code: zip or git.

* izi zip can be downloaded from  our nightly_ server or our Download_  page,
...
* git allows simpler update and easier switching between different versions
  of izi. It also simplifies maintaining non-module patches and
  contributions.  The primary drawback of git is that it is significantly
  larger than a tarball as it contains the entire history of the izi project.

Community Edition
'''''''''''''''''

The git repository is https://github.com/izi/izi.git for the Community
edition.
...
.. code-block:: console
    ...
    $ git clone https://github.com/izi/izi.git
...
.. code-block:: console
  ...
  $ git clone https://github.com/izi/enterprise.git

.. note:: The Enterprise git repository **does not contain the full izi
...
Once all dependencies are set up, izi can be launched by running ``izi-bin``.
...
Under Windows a typical way to execute izi would be:

.. code-block:: doscon

    C:\YouriziPath> python3 izi-bin -w izi -r izi --addons-path=addons,../mymodules --db-filter=mydb$

Where ``izi``, ``izi`` are the postgresql login and password,
``../mymodules`` a directory with additional addons and ``mydb`` the default
db to serve on localhost:8069

Under Unix a typical way to execute izi would be:

.. code-block:: console

    $ ./izi-bin --addons-path=addons,../mymodules --db-filter=mydb$
...
This section will explain how to run izi in a such isolated Python environment.
...
This section assumes that you obtained the izi sources from the zip file or the
...
Now, let's install the tools required to build izi dependencies if needed:
...
Now we can create a virtual environment for izi like this:

.. code-block:: console

  $ mkvirtualenv -p /usr/bin/python3 izi-venv

With this command, we ask for an isolated Python3 environment that will be named
"izi-env". If the command works as expected, your shell is now using this
...
Now let's install the izi required python packages:

.. code-block:: console

  $ cd your_izi_sources_path
  $ pip install -r requirements.txt

After a little while, you should be ready to run izi from the command line as
explained above.
...
Whenever you want to work again with your 'izi-venv' environment:
...
The full documentation on how to use izi with Docker can be found on the
offcial izi `docker image <https://registry.hub.docker.com/_/izi/>`_ page.

.. _demo: https://demo.izi.asia
.. _docker: https://www.docker.com
.. _Download: https://www.izi.asia/page/download
...
.. _saas: https://www.izi.asia/page/start
...
.. _the repository: https://github.com/izi/izi
.. _git: http://git-scm.com
.. _Editions: https://www.izi.asia/pricing#pricing_table_features
.. _nightly: https://nightly.izi.asia/11.0/nightly/
.. _extra: https://nightly.izi.asia/extra/
