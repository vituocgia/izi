    ...
    if hasattr(app, 'set_translator'):
        ...
        if getattr(app.config, 'html_translator_class', None):
            app.warn("Overriding the explicitly set  html_translator_class setting",
                     location="izi extension")
        app.config.html_translator_class = 'izi_ext.translator.BootstrapTranslator'

    switcher.setup(app)
    app.add_config_value('izi_cover_default', None, 'env')
    app.add_config_value('izi_cover_external', {}, 'env')
    app.add_config_value('izi_cover_default_external', lambda conf: conf.izi_cover_default, 'env')
...
def update_meta(app, pagename, templatename, context, doctree):
    ...
    meta.setdefault('banner', app.config.izi_cover_default)
