        ...
        for title, ref in ((e[0], e[1]) for e in node['entries']):
            ...
            for subtitle, subref in entries:
                ...
                if subref in env.metadata:
                    cover = env.metadata[subref].get('banner', conf.izi_cover_default)
                elif subref in conf.izi_cover_external:
                    cover = conf.izi_cover_external[subref]
                else:
                    cover = conf.izi_cover_default_external
