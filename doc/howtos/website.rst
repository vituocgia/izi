...
.. warning::
    ...
    * This guide assumes :ref:`an installed izi <setup/install>`

Creating a basic module
=======================

In izi, tasks are performed by creating modules.

Modules customize the behavior of an izi installation, either by adding new
behaviors or by altering existing ones (including behaviors added by other
modules).

:ref:`izi's scaffolding <reference/cmdline/scaffold>` can setup a basic
module. To quickly get started simply invoke:

.. code-block:: console

    $ ./izi-bin scaffold Academy my-modules
...
* start the izi server

  .. code-block:: console

      $ ./izi-bin --addons-path addons,my-modules
...
display logic. izi allows any Python templating system, but provides its
...
Finally restart izi and update the module's data (to install the template)
by going to :menuselection:`Settings --> Modules --> Modules -->
Academy` and clicking :guilabel:`Upgrade`.

.. tip::

    Alternatively, izi can be restarted :option:`and update modules at
    the same time<izi-bin -u>`:

    .. code-block:: console

        $ izi-bin --addons-path addons,my-modules -d academy -u academy
...
:ref:`izi models <reference/orm/model>` map to database tables.
...
#. Because :meth:`~izi.models.Model.search` returns a set of records
...
izi bundles a module dedicated to building websites.

So far we've used controllers fairly directly, but izi 8 added deeper
...
You're now in izi "proper": the administrative interface. For now click on
...
:func:`~izi.http.route` decorator which takes a routing string and a
...
restart izi, access http://localhost:8069/academy/Alice/ and
...
Restart izi, access http://localhost:8069/academy/2, note how the old value
...
izi provides an additional converter called ``model`` which provides records
...
Restart izi and upgrade the module, then you can visit each teacher's page.
...
Restart izi and update the views, reload the teacher's page and… the field
...
Restart izi and upgrade the module, there is now a placeholder under the
...
A brief and incomplete introduction to the izi administration
--------------------------------------------------------------

The izi administration was briefly seen during the `website support`_ section.
We can go back to it using :menuselection:`Administrator --> Administrator` in
the menu (or :guilabel:`Sign In` if you're signed out).

The conceptual structure of the izi backend is simple:

#. first are menus, a tree (menus can have sub-menus) of records. Menus
   without children map to…
#. actions. Actions have various types: links, reports, code which izi should
   execute or data display. Data display actions are called *window actions*,
   and tell izi to display a given *model* according to a set of views…
...
Editing in the izi administration
----------------------------------

By default, an izi model is essentially invisible to a user. To make it
...
:ref:`view <reference/views>`) izi will automatically create a basic one
...
:class:`the inverse relationship <izi.fields.One2many>` to the *teachers*
model:
...
izi provides technical models, which don't directly fulfill business needs
but which add capabilities to business objects without having to build
them by hand.

One of these is the *Chatter* system, part of izi's email and messaging
system, which can add notifications and discussion threads to any model.
The model simply has to :attr:`~izi.models.Model._inherit`
...
izi also provides business models which allow using or opting in business
needs more directly. For instance the ``website_sale`` module sets up an
e-commerce site based on the products in the izi system. We can easily make
...
restart izi, update your module, there is now a :guilabel:`Shop` section in
...
.. note::
    ...
    * to extend a model in-place, it's :attr:`inherited
      <izi.models.Model._inherit>` without giving it a new
      :attr:`~izi.models.Model._name`
