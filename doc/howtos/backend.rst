...
Start/Stop the izi server
==========================

izi uses a client/server architecture in which clients are web browsers
accessing the izi server via RPC.
...
In order to start the server, simply invoke the command :ref:`izi-bin
...
izi modules can either add brand new business logic to an izi system, or
alter and extend existing business logic: a module can be created to add your
country's accounting rules to izi's generic accounting support, while the
next module adds support for real-time visualisation of a bus fleet.

Everything in izi thus starts and ends with modules.

Composition of a module
-----------------------

An izi module can contain a number of elements:

Business objects
    Declared as Python classes, these resources are automatically persisted
    by izi based on their configuration
...
are specified by using the :option:`--addons-path <izi-bin --addons-path>`
option.
...
An izi module is declared by its :ref:`manifest <reference/module/manifest>`.
...
izi provides a mechanism to help set up a new module, :ref:`izi-bin
<reference/cmdline/server>` has a subcommand :ref:`scaffold
<reference/cmdline/scaffold>` to create an empty module:

.. code-block:: console

    $ izi-bin scaffold <module name> <where to put it>
...
.. exercise:: Module creation
    ...
    .. only:: solutions
        ...
        #. Invoke the command ``izi-bin scaffold openacademy addons``.
...
A key component of izi is the :abbr:`ORM (Object-Relational Mapping)` layer.
...
:class:`~izi.models.Model` which integrates them into the automated
persistence system.

Models can be configured by setting a number of attributes at their
definition. The most important attribute is
:attr:`~izi.models.Model._name` which is required and defines the name for
the model in the izi system. Here is a minimally complete definition of a
model::
...
:attr:`~izi.fields.Field.string` (``unicode``, default: field's name)
    The label of the field in UI (visible by users).
:attr:`~izi.fields.Field.required` (``bool``, default: ``False``)
    If ``True``, the field can not be empty, it must either have a default
    value or always be given a value when creating a record.
:attr:`~izi.fields.Field.help` (``unicode``, default: ``''``)
    Long-form, provides a help tooltip to users in the UI.
:attr:`~izi.fields.Field.index` (``bool``, default: ``False``)
    Requests that izi create a `database index`_ on the column.
...
Example of simple fields are :class:`~izi.fields.Boolean`,
:class:`~izi.fields.Date`, :class:`~izi.fields.Char`.

Reserved fields
###############

izi creates a few fields in all models\ [#autofields]_. These fields are
managed by the system and shouldn't be written to. They can be read if
useful or necessary:

:attr:`~izi.fields.Model.id` (:class:`~izi.fields.Id`)
    The unique identifier for a record in its model.
:attr:`~izi.fields.Model.create_date` (:class:`~izi.fields.Datetime`)
    Creation date of the record.
:attr:`~izi.fields.Model.create_uid` (:class:`~izi.fields.Many2one`)
    User who created the record.
:attr:`~izi.fields.Model.write_date` (:class:`~izi.fields.Datetime`)
    Last modification date of the record.
:attr:`~izi.fields.Model.write_uid` (:class:`~izi.fields.Many2one`)
    user who last modified the record.

Special fields
##############

By default, izi also requires a ``name`` field on all models for various
display and search behaviors. The field used for these purposes can be
overridden by setting :attr:`~izi.models.Model._rec_name`.
...
izi is a highly data driven system. Although behavior is customized using
Python_ code part of a module's value is in the data it sets up when loaded.

.. tip:: some modules exist solely to add data into izi
    :class: aphorism
...
* ``model`` is the name of the izi model for the record.
...
.. tip:: The content of the data files is only loaded when a module is
    ...
    :ref:`izi-bin -u openacademy <reference/cmdline>` to save the changes
    to your database.
...
If no search view exists for the model, izi generates one which only allows
...
:class:`Many2one(other_model, ondelete='set null') <izi.fields.Many2one>`
...
:class:`One2many(other_model, related_field) <izi.fields.One2many>`
    A virtual relationship, inverse of a :class:`~izi.fields.Many2one`.
    A :class:`~izi.fields.One2many` behaves as a container of records,
    ...
    .. danger::
        ...
        Because a :class:`~izi.fields.One2many` is a virtual relationship,
        there *must* be a :class:`~izi.fields.Many2one` field in the
        :samp:`{other_model}`, and its name *must* be :samp:`{related_field}`

:class:`Many2many(other_model) <izi.fields.Many2many>`
...
izi provides two *inheritance* mechanisms to extend an existing model in a
modular way.
...
.. seealso::
    ...
    * :attr:`~izi.models.Model._inherit`
    * :attr:`~izi.models.Model._inherits`

View inheritance
----------------

Instead of modifying existing views in place (by overwriting them), izi
...
In izi, :ref:`reference/orm/domains` are values that encode conditions on
...
:attr:`~izi.fields.Field.compute` to the name of a method. The computation
...
on the compute method with the decorator :func:`~izi.api.depends`.
    ...
    * Add a field ``active`` in the class Session, and set sessions as active by
    ...
    .. only:: solutions
        ...
        .. note::
            ...
            izi has built-in rules making fields with an ``active`` field set
            to ``False`` invisible.
...
the record in the form view, and decorate it with :func:`~izi.api.onchange`
...
izi provides two ways to set up automatically verified invariants:
:func:`Python constraints <izi.api.constrains>` and
:attr:`SQL constraints <izi.models.Model._sql_constraints>`.

A Python constraint is defined as a method decorated with
:func:`~izi.api.constrains`, and invoked on a recordset. The decorator
...
:attr:`~izi.models.Model._sql_constraints`. The latter is assigned to a list
...
.. exercise:: Add access control through the izi interface
...
the same convention as the method :meth:`~izi.models.Model.write` of the ORM.
...
:class:`~izi.models.TransientModel` instead of
:class:`~izi.models.Model`. The class
:class:`~izi.models.TransientModel` extends :class:`~izi.models.Model`
...
pt_BR.po). Translations will be loaded automatically by izi for all
enabled languages. Developers always use English when creating a module, then
export the module terms using izi's gettext POT export feature
...
.. tip:: The Portable Object files generated by izi are published on
         `Transifex <https://www.transifex.com/izi/public/>`__, making it
...
.. code-block:: text
   ...
   |- idea/ # The module directory
      |- i18n/ # Translation files
         | - idea.pot # Translation Template (exported from izi)
...
.. tip:: 
   ...
   By default izi's POT export only extracts labels inside XML files or
   inside field definitions in Python code, but any Python string can be
   translated this way by surrounding it with the function :func:`izi._`
   (e.g. ``_("Label")``)

.. exercise:: Translate a module

   Choose a second language for your izi installation. Translate your
   module using the facilities provided by izi.
   ...
   .. only:: solutions
        ...
        #. In ``models.py``, add an import statement for the function
           ``izi._`` and mark missing strings as translatable
...
izi 11.0 uses a report engine based on :ref:`reference/qweb`,
...
izi is accessible through XML-RPC/JSON-RPC interfaces, for which libraries
exist in many languages.

XML-RPC Library
---------------

The following example is a Python 3 program that interacts with an izi
...
.. exercise:: Add a new service to the client
   ...
   izi (yours, or your instructor's). This program should display all
...
The following example is a Python 3 program that interacts with an izi server
...
.. note::
    ...
    There are a number of high-level APIs in various languages to access izi
