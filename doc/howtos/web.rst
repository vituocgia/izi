...
This guide is about creating modules for izi's web client.

To create websites with izi, see :doc:`website`; to add business capabilities
or extend existing business systems of izi, see :doc:`backend`.
...
.. warning::
    ...
    * `Underscore.js`_
    ...
    It also requires :ref:`an installed izi <setup/install>`, and Git_.


A Simple Module
===============

Let's start with a simple izi module holding basic web component
...
.. code-block:: console
    ...
    $ git clone http://github.com/izi/petstore

This will create a ``petstore`` folder wherever you executed the command.
You then need to add that folder to izi's
:option:`addons path <izi-bin --addons-path>`, create a new database and
...
Files used in the "web" side of an izi module must be placed in a ``static``
...
``oepetstore/static/js/petstore.js``
    ...
    currently look like::
        ...
        izi.oepetstore = function(instance, local) {
...
.. warning::
    ...
    "developer mode": log into your izi instance (user *admin* password
    *admin* by default) open the user menu (in the top-right corner of the
    izi screen) and select :guilabel:`About izi` then :guilabel:`Activate
    the developer mode`:

    .. image:: web/about_izi.png
        :align: center
...
The izi framework uses one such pattern to define modules within web addons,
...
In izi web, modules are declared as functions set on the global ``izi``
...
* the first parameter is the current instance of the izi web client, it gives
  access to various capabilities defined by the izi (translations,
...
* the second parameter is your own local namespace automatically created by
  ...
  outside your module (either because the izi web client needs to call them
...
For simplicity and developer-friendliness izi web provides a class
system based on John Resig's `Simple JavaScript Inheritance`_.

New classes are defined by calling the :func:`~izi.web.Class.extend`
method of :class:`izi.web.Class`::
...
The :func:`~izi.web.Class.extend` method takes a dictionary describing
...
by calling :func:`~izi.web.Class.extend` on the parent class, as is done
to subclass :class:`~izi.web.Class`::
...
The izi web client bundles jQuery_ for easy DOM manipulation. It is useful
...
izi Web makes specific components responsible for sections of a page. In
izi web, the base for such components is the :class:`~izi.Widget`
...
It extends :class:`~izi.Widget` and overrides the standard method
:func:`~izi.Widget.start`, which — much like the previous ``MyClass``
— does little for now.
...
The ``HomePage`` widget already has a :func:`~izi.Widget.start`
...
All widgets have a :attr:`~izi.Widget.$el` which represents the
section of page they're in charge of (as a jQuery_ object). Widget content
should be inserted there. By default, :attr:`~izi.Widget.$el` is an
empty ``<div>`` element.
...
Let's add some content to the widget's root element, using jQuery::
    ...
    local.HomePage = instance.Widget.extend({
        start: function() {
            this.$el.append("<div>Hello dear izi user!</div>");
        },
    });
...
.. note::
    ...
    to refresh the javascript code loaded in izi Web, you will need to reload
    the page. There is no need to restart the izi server.

The ``HomePage`` widget is used by izi Web and managed automatically.
...
``GreetingsWidget``'s :func:`~izi.Widget.appendTo` method::
...
* Finally it tells ``GreetingsWidget`` where to insert itself, delegating part
  of its :attr:`~izi.Widget.$el` to the ``GreetingsWidget``.

When the :func:`~izi.Widget.appendTo` method is called, it asks the
widget to insert itself at the specified position and to display its content.
The :func:`~izi.Widget.start` method will be called during the call
to :func:`~izi.Widget.appendTo`.
...
:class:`~izi.Widget`, because we added some classes on them.
...
When overriding the :func:`~izi.Widget.init` method of a widget it is
...
it. This is done via the :func:`~izi.Widget.destroy` method::

    greeting.destroy();

When a widget is destroyed it will first call
:func:`~izi.Widget.destroy` on all its children. Then it erases itself
from the DOM. If you have set up permanent structures in
:func:`~izi.Widget.init` or :func:`~izi.Widget.start` which
must be explicitly cleaned up (because the garbage collector will not handle
them), you can override :func:`~izi.Widget.destroy`.

.. danger::

    when overriding :func:`~izi.Widget.destroy`, ``_super()``
...
As many other environments, izi's solution is to use a `template engine`_.
izi's template engine is called :ref:`reference/qweb`.
...
* It has special support in izi Web's :class:`~izi.Widget`, though it
  can be used outside of izi's web client (and it's possible to use
  :class:`~izi.Widget` without relying on QWeb)
...
.. note::
    ...
    like izi :ref:`views <reference/views>`.
    ...
    characteristic of izi, and losing it was considered unacceptable.
...
However, because :class:`~izi.Widget` has special integration for QWeb
the template can be set directly on the widget via its
:attr:`~izi.Widget.template` attribute::
...
* with the second version, the template is rendered right before
  :func:`~izi.Widget.start` is called
...
.. warning::
    ...
    they're set as a widget's :attr:`~izi.Widget.template`. If there are
...
When using :class:`~izi.Widget`'s integration it is not possible to
...
before :func:`~izi.Widget.start` is called (the widget's state will
essentially be that set up by :func:`~izi.Widget.init`):
...
But because it's a common operation, :class:`~izi.Widget` provides an
equivalent shortcut through the :func:`~izi.Widget.$` method::
...
:attr:`~izi.Widget.events` is an object (mapping) of an event to the
...
:func:`~izi.Widget.trigger` takes the name of the event to trigger as
...
listening to the ``user_chose`` event using :func:`~izi.Widget.on`::
...
:func:`~izi.Widget.on` binds a function to be called when the
...
additional arguments of :func:`~izi.Widget.trigger` if it has
any. Example::
...
.. note::
    ...
    exception to that rule is ``izi.web.bus`` which exists specifically
    to broadcasts evens in which any widget could be interested throughout
    the izi web application.
...
* :func:`~izi.Widget.set` sets the value of a property and triggers
  :samp:`change:{propname}` (where *propname* is the property name passed as
  first parameter to :func:`~izi.Widget.set`) and ``change``
* :func:`~izi.Widget.get` retrieves the value of a property.
...
The class system of the izi web framework allows direct modification of
existing classes using the :func:`~izi.web.Class.include` method::
...
defined in the call to :func:`~izi.web.Class.include`. This will also work
if some instances of the class (or of any of its sub-classes) were created
prior to the call to :func:`~izi.Widget.include`.
...
In izi, translations files are automatically generated by scanning the source
...
:func:`~izi.web._t` (and also :func:`~izi.web._lt`).
...
:func:`~izi.web._lt` ("lazy translate") is similar but somewhat more
...
Communication with the izi Server
==================================

Contacting Models
-----------------

Most operations with izi involve communicating with *models* implementing
...
communicating with izi requires additional metadata whose setup before every
call would be verbose and error-prone. As a result, izi web provides
...
The class used to call izi models is :class:`izi.Model`. It is
instantiated with the izi model's name as first parameter
(``oepetstore.message_of_the_day`` here).

:func:`~izi.web.Model.call` can be used to call any (public) method of an
izi model. It takes the following positional arguments:
...
:func:`~izi.Widget.call` returns a deferred resolved with the value
...
the time zone of the user, used to compute correctly dates and times if izi
...
solution to achieve that is to use :class:`izi.web.CompoundContext`.

:class:`~izi.web.CompoundContext` is a class used to pass the user's
...
dictionaries or other :class:`~izi.web.CompoundContext` instances. It will
...
are related to the configuration of the current user in izi plus the
``new_key`` key that was added when instantiating
:class:`~izi.web.CompoundContext`.

Queries
-------

While :func:`~izi.Model.call` is sufficient for any interaction with izi
models, izi Web provides a helper for simpler and clearer querying of models
(fetching of records based on various conditions):
:func:`~izi.Model.query` which acts as a shortcut for the common
combination of :py:meth:`~izi.models.Model.search` and
::py:meth:`~izi.models.Model.read`. It provides a clearer syntax to search
and read models::
...
* :func:`~izi.web.Model.query` takes an optional list of fields as
  parameter (if no field is provided, all fields of the model are fetched). It
  returns a :class:`izi.web.Query` which can be further customized before
  being executed
* :class:`~izi.web.Query` represents the query being built. It is
  ...
  :class:`~izi.web.Query` for its customization options.

When the query is set up as desired, simply call
:func:`~izi.web.Query.all` to execute it and return a
deferred to its result. The result is the same as
:py:meth:`~izi.models.Model.read`'s, an array of dictionaries where each
...
.. exercise:: Pet Toys List
    ...
    In izi, images are generally stored in regular fields encoded as
...
In izi, many operations start from an :ref:`action <reference/actions>`:
...
In izi Web, the component responsible for handling and reacting to these
...
:func:`~izi.Widget.do_action` is a shortcut of :class:`~izi.Widget`
...
the izi web know to start this widget? Because the widget is registered as
a *client action*.

A client action is (as its name implies) an action type defined almost
entirely in the client, in javascript for izi web. The server simply sends
...
``instance.web.client_actions`` is a :class:`~izi.web.Registry` in which
the action manager looks up client action handlers when it needs to execute
one. The first parameter of :class:`~izi.web.Registry.add` is the name
(tag) of the client action, and the second parameter is the path to the widget
from the izi web client root.
...
Much of izi web's usefulness (and complexity) resides in views. Each view
...
Most :ref:`izi views <reference/views>` are implemented through a subclass
of :class:`izi.web.View` which provides a bit of generic basic structure
for handling events and displaying model information.

The *search view* is considered a view type by the main izi framework, but
...
:py:class:`~izi.models.Model.fields_view_get`) and any other data source
it needs. To that purpose, views are provided with an optional view
identifier set as the :attr:`~izi.web.View.view_id` attribute.

Views are also provided with a :class:`~izi.web.DataSet` instance which
...
:func:`~izi.web.View.do_search`, and updating their
:class:`~izi.web.DataSet` as necessary.
...
:py:class:`~izi.fields.Char` fields which should contain email addresses
...
* It must correctly implement the 3 field attributes available in all fields
  of izi. The ``AbstractField`` class already implements an algorithm that
  ...
  * ``readonly``: When ``true``, the field must not be editable by the
    user. Most fields in izi have a completely different behavior depending
...
* Fields have two methods, ``set_value()`` and ``get_value()``, which are
  ...
  given by the izi server when a ``read()`` is performed on a model and give
  ...
  ``write()`` is not necessarily the same in izi. As example, when you read a
...
interface and the ``AbstractField`` class directly in the code of the izi web
client.
...
user to modify it can be useful, but most fields in izi also allow editing.
...
their :attr:`~izi.web.form.FormWidget.field_manager` attribute::
...
:attr:`~izi.web.form.FormWidget` is generally the
:class:`~izi.web.form.FormView` itself, but features used from it should
be limited to those defined by :class:`~izi.web.form.FieldManagerMixin`,
the most useful being:

* :func:`~izi.web.form.FieldManagerMixin.get_field_value(field_name)`
  which returns the value of a field.
* :func:`~izi.web.form.FieldManagerMixin.set_values(values)` sets multiple
