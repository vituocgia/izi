...
class GoogleCalendarController(http.Controller):
    ...
    def sync_data(self, model, **kw):
        """ This route/function is called when we want to synchronize izi calendar with Google Calendar
            Function return a dictionary with the status :  need_config_from_admin, need_auth, need_refresh, success if not calendar_event
            The dictionary may contains an url, to allow izi Client to redirect user on this URL for authorization for example
        """
        if model == 'calendar.event':
            ...
            # Checking that user have already accepted izi to access his calendar !
    ...
    def remove_references(self, model, **kw):
        """ This route/function is called when we want to remove all the references between one calendar izi and one Google Calendar """
        status = "NOP"
        if model == 'calendar.event':
            GoogleCal = request.env['google.calendar']
            # Checking that user have already accepted izi to access his calendar !
