...
class SyncEvent(object):
    ...
    def compute_OP(self, modeFull=True):
        #If event are already in Gmail and in izi
        ...
        # New in izi...  Create on create_events of synchronize function
        elif self.OE.found and not self.GG.found:
            if self.OE.status:
                self.OP = Delete('OE', 'Update or delete from GOOGLE')
            else:
                if not modeFull:
                    self.OP = Delete('GG', 'Deleted from izi, need to delete it from Gmail if already created')
                else:
                    self.OP = NothingToDo("", "Already Deleted in gmail and unlinked in izi")
...
class GoogleCalendar(models.AbstractModel):
    ...
    def create_an_event(self, event):
        """ Create a new event in google calendar from the given event in izi.
    ...
    def update_from_google(self, event, single_event_dict, type):
        """ Update an event in izi with information from google calendar
    ...
    def update_events(self, lastSync=False):
        ...
        if lastSync:
            ...
            my_izi_attendees = CalendarAttendee.with_context(context_novirtual).search([
                ('partner_id', '=', my_partner_id),
                ('event_id.oe_update_date', '>', lastSync and fields.Datetime.to_string(lastSync) or self.get_minTime().fields.Datetime.to_string()),
                ('google_internal_event_id', '!=', False),
            ])

            my_izi_googleinternal_records = my_izi_attendees.read(['google_internal_event_id', 'event_id'])

            if self.get_print_log():
                _logger.info("Calendar Synchro -  \n\nUPDATE IN GOOGLE\n%s\n\nRETRIEVE FROM OE\n%s\n\nUPDATE IN OE\n%s\n\nRETRIEVE FROM GG\n%s\n\n", all_event_from_google, my_google_att_ids, my_izi_attendees.ids, my_izi_googleinternal_records)

            for gi_record in my_izi_googleinternal_records:
            ...
            my_attendees = (my_google_attendees | my_izi_attendees)
            ...
            # Select all events from izi which have been already synchronized in gmail
    ...
    def authorize_google_uri(self, from_url='http://www.izi.asia'):
