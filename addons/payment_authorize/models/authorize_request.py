...
class AuthorizeAPI():
    ...
    def create_customer_profile(self, partner, cardnumber, expiration_date, card_code):
        ...
        a corresponding payment profile to it. Note that a single partner in the izi
        ...
        etree.SubElement(profile, "merchantCustomerId").text = 'izi-%s-%s' % (partner.id, uuid4().hex[:8])
    ...
    def create_customer_profile_from_tx(self, partner, transaction_id):
        ...
        etree.SubElement(customer, "merchantCustomerId").text = 'izi-%s-%s' % (partner.id, uuid4().hex[:8])
