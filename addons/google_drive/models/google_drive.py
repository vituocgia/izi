...
from izi.exceptions import RedirectWarning, UserError
from izi.tools import pycompat
from izi.tools.safe_eval import safe_eval
from izi.tools.translate import _

from izi.addons.google_account.models.google_service import GOOGLE_TOKEN_ENDPOINT, TIMEOUT
...
class GoogleDrive(models.Model):
    ...
    def copy_doc(self, res_id, template_id, name_gdocs, res_model):
        ...
        if content.get('alternateLink'):
            ...
            request_url = "https://www.googleapis.com/drive/v2/files/%s/permissions?emailMessage=This+is+a+drive+file+created+by+izi&sendNotificationEmails=false&access_token=%s" % (key, access_token)
