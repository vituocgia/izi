...
def configure_parser():
    ...
    group = optparse.OptionGroup(parser, "Note",
        ...
        "with the izi server for case management in the CRM module.")
    parser.add_option_group(group)
    parser.add_option("-u", "--user", dest="userid",
                      help="izi user id to connect with",
                      default=config.OPENERP_DEFAULT_USER_ID, type='int')
    parser.add_option("-p", "--password", dest="password",
                      help="izi user password",
    ...
    parser.add_option("-d", "--dbname", dest="dbname",
                      help="izi database name (default: %default)",
                      default=config.OPENERP_DEFAULT_DATABASE)
    parser.add_option("--host", dest="host",
                      help="izi Server hostname",
                      default=config.OPENERP_HOSTNAME)
    parser.add_option("--port", dest="port",
                      help="izi Server XML-RPC port number",
...
def main():
    """
    Receive the email via the stdin and send it to the izi Server
    """
    ...
    try:
        ...
        subject = '[izi]:ERROR: Mailgateway - %s' % time.strftime('%Y-%m-%d %H:%M:%S')
        send_mail(
            config.MAIL_ERROR,
            config.MAIL_ADMINS,
            subject, msg, files=[('message.txt', msg_txt)]
        )
        sys.stderr.write("Failed to deliver email to izi Server, sending error notification to %s\n" % config.MAIL_ADMINS)
