...
class Alias(models.Model):
    """A Mail Alias is a mapping of an email address with a given izi Document
       model. It is used by izi's mail gateway when processing incoming emails
       ...
       created, it becomes immediately usable and izi will accept email for it.
     """
    _name = 'mail.alias'
    ...
    alias_name = fields.Char('Alias Name', help="The name of the email alias, e.g. 'jobs' if you want to catch emails for <jobs@example.izi.asia>")
    alias_model_id = fields.Many2one('ir.model', 'Aliased Model', required=True, ondelete="cascade",
                                     help="The model (izi Document Kind) to which this alias "
    ...
    def name_get(self):
        """Return the mail alias display alias_name, including the implicit
           ...
           e.g. `jobs@mail.izi.asia` or `jobs` or 'New Alias'
        """
        res = []
        for record in self:
