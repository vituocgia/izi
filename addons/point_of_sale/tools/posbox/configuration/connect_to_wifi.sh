...
function connect () {
	...
	if [ ${TIMEOUT_RETURN} -eq 124 ] && [ -z "${NO_AP}" ] ; then
		...
		sudo /home/pi/izi/addons/point_of_sale/tools/posbox/configuration/wireless_ap.sh "force" &
	else
		...
		if [ ! -f "${LOST_WIFI_FILE}" ] ; then
			logger -t posbox_connect_to_wifi "Restarting izi"
			sudo service izi restart
		fi
		...
		/home/pi/izi/addons/point_of_sale/tools/posbox/configuration/keep_wifi_alive.sh &
	fi
}
