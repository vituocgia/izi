...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/izi"
...
if [ ! -d $CLONE_DIR ]; then
    ...
    mkdir -p "${CLONE_DIR}"
    git clone -b 8.0 --no-local --no-checkout --depth 1 https://github.com/izi/izi.git "${CLONE_DIR}"
    cd "${CLONE_DIR}"
...
openerp/
izi.py" | tee --append .git/info/sparse-checkout > /dev/null
