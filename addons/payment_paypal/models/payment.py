...
from izi.addons.payment_paypal.controllers.main import PaypalController
...
class TxPaypal(models.Model):
    ...
    def _paypal_form_get_invalid_parameters(self, data):
        ...
        if not data.get('receiver_id') or not self.acquirer_id.paypal_seller_account:
            ...
            # In izi, there is only one field for the Paypal email: the business email. This isn't possible to set a receiver_email
            # different than the business email. Therefore, if you want such a configuration in your Paypal, you are then obliged to fill
            # the Merchant ID in the Paypal payment acquirer in izi, so the check is performed on this variable instead of the receiver_email.
