define([
  ...
  var Renderer = function () {
    ...
    this.tplButtonInfo = tplButtonInfo; // izi: allow access for override
    this.tplPopovers = tplPopovers; // izi: allow access for override
    ...
    this.createPalette = createPalette; // izi: allow access for override
    ...
    this.createLayoutByAirMode = function ($holder, options) {
      ...
      $holder.addClass('note-air-editor note-editable'); // izi: removing panel-body class to remove unwanted style
      $holder.attr({
        'data-note-id': id, // izi: we use [data-note-id="{id}"] instead of [id="{id}"]
        // 'id': 'note-editor-' + id,
      ...
      // create Popover
      var $popover = $(this.tplPopovers(langInfo, options)); // izi: user (maybe) overrided method
      $popover.addClass('note-air-layout');
      $popover.attr('id', 'note-popover-' + id);
      $popover.appendTo($container);
      createTooltip($popover, keyMap);
      this.createPalette($popover, options); // izi: use (maybe) overrided method
    ...
    this.createLayoutByFrame = function ($holder, options) {
      ...
      //04. create Popover
      var $popover = $(this.tplPopovers(langInfo, options)).prependTo($editingArea); // izi: use (maybe) overrided method
      this.createPalette($popover, options); // izi: use (maybe) overrided method
      createTooltip($popover, keyMap);
      ...
      var keyMap = options.keyMap[agent.isMac ? 'mac' : 'pc'];
      this.createPalette($toolbar, options); // izi: use (maybe) overrided method
    ...
    this.removeLayout = function ($holder, layoutInfo, options) {
      if (options.airMode) {
        $holder.removeClass('note-air-editor note-editable')
               .removeAttr('contentEditable'); // izi: removed id 'id contentEditable'
    ...
    this.getTemplate = function () {
      return {
        ...
        dialog: tplDialog,
        dropdown: tplDropdown // izi: suggest upstream
      };
    };
