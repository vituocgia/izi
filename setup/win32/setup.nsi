...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\izi Server ${VERSION}"
...
InstallDir "$PROGRAMFILES\izi ${VERSION}"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.izi.asia/page/contactus"
...
; English
LangString DESC_OpenERP_Server ${LANG_ENGLISH} "Install the izi Server with all the izi standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by izi."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact izi for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the izi Server"
...
LangString TITLE_OpenERP_Server ${LANG_ENGLISH} "izi Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start izi"

; French
LangString DESC_OpenERP_Server ${LANG_FRENCH} "Installation du Serveur izi avec tous les modules izi standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par izi."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez izi pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur izi"
...
LangString TITLE_OpenERP_Server ${LANG_FRENCH} "Serveur izi"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer izi"
...
# If there is a previous install of the OpenERP Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\izi.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\izi.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\izi.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\izi.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\izi.conf" "options" "addons_path" "$INSTDIR\server\izi\addons"
    WriteIniStr "$INSTDIR\server\izi.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\izi.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\izi-bin" --stop-after-init --logfile "$INSTDIR\server\izi.log" -s'
    ${If} ${RunningX64}
      nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe" "\"$INSTDIR\server\izi-bin\""'
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\${POSTGRESQL_EXE_FILENAME}" \
        ...
        --servicename "PostgreSQL_For_izi" \
...
;    WriteRegDWORD HKLM     "${UNINSTALL_REGISTRY_KEY}" "VersionMinor" "${REVISION_VERSION}"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@izi.asia"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.izi.asia"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@izi.asia"
